#!/usr/bin/env python3
"""Train diffusion TWM v9: continuous embedding-space noise.

Replaces discrete masking with DDPM-style Gaussian corruption in embedding
space. The noise process is continuous — no discontinuity between "denoising"
and "generation" tasks. Timestep conditioning tells the denoiser what noise
level it's working at.

Key changes from v7:
  - Continuous noise process replaces discrete masking
  - Cosine noise schedule (DDPM-style)
  - Timestep conditioning via sinusoidal embedding added to adaLN context
  - Loss on ALL positions (not just masked ones)
  - Loss-vs-timestep diagnostic every N epochs

Usage:
    uv run python scripts/train_diffusion_v9.py \
        --data-dir data/atomic_10000 \
        --out-dir results/v9_continuous_simple \
...
def _diffusion_loss_continuous(logits, target_mask, target_ids, pad_mask, B, M):
    ...
    # target_mask is all-True for continuous mode, so logits_at_mask == logits
...
def compute_loss(
    ...
    # Entity diffusion (continuous noise)
    ...
    # Value diffusion (continuous noise)
...
def run_loss_vs_timestep(
    ...
    Should show smooth, monotonically increasing loss curve (higher t = more noise = harder).
    """
...
def main():
    ap = argparse.ArgumentParser(description="Train diffusion TWM v9 (continuous noise)")
    ...
    ap.add_argument("--alpha-min", type=float, default=0.0,
                    help="Minimum alpha (signal floor). >0 avoids singularity at t=1.0")
    ap.add_argument("--timestep-bias-power", type=float, default=1.0,
                    help="Importance sampling bias (>1 = more high-t samples, 1=uniform)")
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        for start in range(0, n_train - args.batch_size + 1, args.batch_size):
            ...
            # Timestep sampled uniformly per example inside the decoder
            loss, _ = compute_loss(
                model, latent,
