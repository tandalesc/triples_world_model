#!/usr/bin/env python3
"""Train diffusion TWM with domain-specific word-level vocabulary.

Replaces 32K T5 vocab with ~8K domain words from ATOMIC training data.
Sequences drop from 16 T5 subwords to ~6 words. Same architecture otherwise.

Usage:
    uv run python scripts/train_diffusion_v4.py \
        --data-dir data/atomic_10000 \
        --out-dir results/v4_vocab \
...
from twm.domain_vocab import DomainVocab, MASK_ID
...
def eval_generation(
    ...
    """Run generation-from-scratch quality measurement."""
    model.eval()
    ...
    for i in range(n):
        for m in range(M):
            ...
            tgt_e = ds._entity_phrases[i][m]
            pred_e = gen_entities[i][m]
            if pred_e == tgt_e:
                entity_match += 1

            tgt_v = ds._value_phrases[i][m]
...
def main():
    ap = argparse.ArgumentParser(description="Train diffusion TWM with domain vocab")
    ...
    ap.add_argument("--max-word-tokens", type=int, default=10)
    ap.add_argument("--min-word-count", type=int, default=3)
    ...
    # Phrase vocab (for attr discrete head)
    ...
    phrase_vocab = PhraseVocab()
    phrase_vocab.build(train_examples)
    for role, size in phrase_vocab.vocab_sizes.items():
        print(f"  {role}: {size} phrases")
    phrase_vocab.save(out_dir / "phrase_vocab.json")

    # Domain vocab (for entity/value diffusion decoders)
    print("Building domain vocabulary...")
    domain_vocab = DomainVocab.from_training_data(train_path, min_count=args.min_word_count)
    print(f"  Domain vocab: {domain_vocab.vocab_size} words (min_count={args.min_word_count})")
    domain_vocab.save(out_dir / "domain_vocab.json")

    # Model
    print("Building diffusion model...")
    model = DiffusionWorldModel(
        config, st_dim, phrase_vocab,
        max_value_tokens=args.max_word_tokens,
        ...
        token_vocab_size=domain_vocab.vocab_size,
        mask_token_id=MASK_ID,
        tokenizer=domain_vocab,
    ).to(device)
    ...
    print(f"  Token vocab: {domain_vocab.vocab_size}, max_seq_len: {args.max_word_tokens}")
    print(f"  Projection: {args.n_proj_tokens} tokens")
    print(f"  Total: {model.param_count():,} params, {model.trainable_param_count():,} trainable", flush=True)

    # Dataset
    print("Building datasets...")
    train_ds = DomainTripleDataset(
        train_path, encode_fn, phrase_vocab, domain_vocab,
    ...
    if test_path.exists():
        test_ds = DomainTripleDataset(
            test_path, encode_fn, phrase_vocab, domain_vocab,
            max_triples=config.max_triples,
            max_word_tokens=args.max_word_tokens,
        )
    ...
    with open(out_dir / "model_config.json", "w") as f:
        json.dump({
            ...
            "domain_vocab_size": domain_vocab.vocab_size,
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.log_every == 0 or epoch == 1:
            ...
            if test_ds is not None:
                ...
                # Generation-from-scratch measurement
                gen_metrics = eval_generation(
                    model, test_ds, device, domain_vocab,
                ...
                # Track best gen_val
                gv = gen_metrics.get("gen_val", 0)
                if gv > best_gen_val:
                    best_gen_val = gv
                    torch.save(model.state_dict(), out_dir / "model_best_gen.pt")
            ...
            if epoch % (args.log_every * 5) == 0 or epoch == 1:
                ...
                for i in range(n_gen):
                    ...
                    for m in range(M):
                        if not tgt_pad[i, m]:
                            tgt_e = train_ds._entity_phrases[i][m]
                            tgt_v = train_ds._value_phrases[i][m]
                            pred_e = gen_entities[i][m]
                            pred_v = gen_values[i][m]
            ...
            if epochs_without_improvement >= args.patience and test_ds is not None:
                print(f"\nEarly stopping at epoch {epoch} (best test loss {best_test_loss:.4f} at epoch {best_epoch})", flush=True)
                break
    ...
    print(f"Best gen_val: {best_gen_val:.4f}")
