#!/usr/bin/env python3
"""Train diffusion TWM v12d: MSE x₀-prediction in W-space.

The denoiser predicts clean embedding vectors directly (x₀-prediction)
instead of classification logits. Loss is MSE in embedding space. Inference
uses nearest-neighbor lookup against token embeddings. The output head is
eliminated — the entire pipeline is continuous.

This dissolves the discrimination/construction split: at every noise level
the model answers "where in W-space is the clean embedding?" rather than
learning qualitatively different strategies at different noise levels.

Changes from v12:
  - No output_head (saves ~384K params)
  - MSE loss on predicted clean embeddings vs target clean embeddings
  - Inference: NN cosine lookup against token_emb.weight
  - Intermediate denoising steps are fully continuous (no argmax)
  - New metrics: gen_val_mse, gen_val_cos (continuous quality measures)
...
def _mse_loss(pred_emb, target_mask, target_ids, pad_mask, B, M, token_emb):
    """MSE loss between predicted clean embeddings and target token embeddings."""
    ...
    # Mask: only loss on non-pad token positions
    # target_mask is all-True for continuous mode, so pred_emb shape = (n_valid, S, d_model)
    # But we still need to ignore pad tokens (id=0)
    ...
    # Metrics: cosine sim and NN accuracy
...
def _ce_loss(logits, target_mask, target_ids, pad_mask, B, M):
    """Standard CE loss (for attr head)."""
...
def compute_loss(
    ...
    # Entity: MSE on predicted clean embeddings
    ...
    # Value: MSE on predicted clean embeddings
...
def run_continuous_gen_metrics(
    ...
    """Compute continuous generation metrics: MSE and cosine sim at t=1.0.

    Runs a single forward pass at t=1.0 (pure noise) and measures how close
    the predicted clean embeddings are to the actual targets. This gives a
    smoother signal than token accuracy, especially early in training.
    """
    n = min(n_examples, len(ds))
    latent = model.encode_dynamics(
        ds._all_inputs[:n].to(device),
    ...
    t_tensor = torch.ones(B, device=device)  # t=1.0
    ...
    for role, tgt_ids, prefix in [("value", val_ids, "val"), ("entity", ent_ids, "ent")]:
        ...
        results[f"gen_{prefix}_mse"] = F.mse_loss(pred_flat, target_flat).item()
...
def run_loss_vs_timestep(
    ...
    for t_val in timesteps:
        ...
        if val_pred.shape[0] > 0 and val_mask.any():
            ...
            non_pad = tgt_valid != 0
            if non_pad.any():
                mse = F.mse_loss(val_pred[non_pad], target_clean[non_pad]).item()
        ...
        results[f"t_{t_val:.1f}_mse"] = mse
...
def run_conditioning_reliance(
    ...
    """Conditioned vs unconditioned MSE gap at each timestep."""
    if timesteps is None:
    ...
    for t_val in timesteps:
        ...
        mse_cond = F.mse_loss(pred_cond[non_pad], target_clean[non_pad]).item()

        # Unconditioned
        pred_uncond, _ = decoder(ctx_zero, tgt_valid, timestep=t_tensor, role_id=role_id)
        mse_uncond = F.mse_loss(pred_uncond[non_pad], target_clean[non_pad]).item()

        results[f"t_{t_val:.1f}_cond"] = mse_cond
        results[f"t_{t_val:.1f}_uncond"] = mse_uncond
        results[f"t_{t_val:.1f}_gap"] = mse_uncond - mse_cond

    return results


def main():
    ap = argparse.ArgumentParser(description="Train diffusion TWM v12d (MSE x₀-prediction)")
    ...
    print(f"Prediction: MSE x₀-prediction (no output head)")
    ...
    print("Building diffusion model (MSE x₀-prediction)...")
    model = DiffusionWorldModel(
        config, st_dim, vocab,
    ...
    if test_ds is not None and args.diagnostic_every > 0:
        ...
        diag_line = "  ".join(
            f"t={k.split('_')[1]}: mse={diag[k]:.4f} cos={diag[k.replace('_mse','_cos')]:.3f}"
            for k in sorted(diag.keys()) if k.endswith("_mse")
        )
        print(f"  DIAG: {diag_line}")

        cont = run_continuous_gen_metrics(model, test_ds, device, n_examples=64)
        if cont:
            print(f"  GEN (t=1.0): val_mse={cont.get('gen_val_mse', 0):.4f} "
                  f"val_cos={cont.get('gen_val_cos', 0):.3f} "
                  f"ent_mse={cont.get('gen_ent_mse', 0):.4f} "
    ...
    for epoch in range(1, args.epochs + 1):
        ...
        if epoch % args.log_every == 0 or epoch == 1:
            ...
            if test_ds is not None:
                ...
                if cont_metrics:
                    log += (
                        f" | mse_v {cont_metrics.get('gen_val_mse', 0):.4f}"
                        f" cos_v {cont_metrics.get('gen_val_cos', 0):.3f}"
                    )
