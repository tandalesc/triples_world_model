...
COMMON="--data-dir data/atomic_10000 \
    ...
    --cond-drop-prob 0.15 \
    --token-level --max-tokens-per-slot 12 \
    --device cuda:1"
...
# Token-level CE output head
echo ">>> v12g_token_ce: token-level input, CE output head"
uv run python scripts/train_diffusion_v12g.py \
    --out-dir results/v12g_token_ce \
