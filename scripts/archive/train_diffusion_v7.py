#!/usr/bin/env python3
"""Train diffusion TWM v7: adaLN-Zero conditioning.

Key changes from v6:
  - adaLN-Zero (DiT-style adaptive layer normalization) replaces FiLM
  - Zero-initialized gates so layers start as identity
  - All v6 features: gen_val_tok early stopping, dual metrics, diversity tracking
...
def main():
    ap = argparse.ArgumentParser(description="Train diffusion TWM v7 (adaLN-Zero conditioning)")
    ...
    ap.add_argument("--use-adaln", action="store_true",
                    help="Use adaLN-Zero (DiT-style) conditioning")
    ...
    print(f"FiLM: {args.use_film}, adaLN: {args.use_adaln}, Cross-attention: {use_cross_attention}")
