#!/usr/bin/env python3
"""Train diffusion TWM v11: domain-specific BPE vocabulary.

Replaces T5's 32K vocabulary with a ~1500-token BPE vocabulary trained on
ATOMIC data. Embeddings are 128d (matching denoiser d_model) and fully
trainable — no frozen T5 embeddings, no projection layer.

Key changes from v10:
  - Domain BPE tokenizer (~1500 tokens vs 32K)
  - Trainable 128d embeddings (native denoiser dimension)
  - Shorter output sequences (12 tokens vs 16)
  - Output head: Linear(128, ~1500) = 192K vs Linear(128, 32100) = 4.1M
  - Total trainable: ~2.75M vs ~6.5M
...
def run_eval_generation(
    ...
    """Generation from scratch with token-level and exact-match metrics."""
...
def main():
    ap = argparse.ArgumentParser(description="Train diffusion TWM v11 (domain BPE vocab)")
    ap.add_argument("--data-dir", type=str, required=True)
    ap.add_argument("--out-dir", type=str, required=True)
    ap.add_argument("--domain-tokenizer", type=str, required=True,
                    help="Path to domain BPE tokenizer JSON")
    ...
    ap.add_argument("--t5-init-embeddings", action="store_true",
                    help="Initialize domain embeddings from T5 via PCA projection")
    ...
    print(f"Domain BPE vocab: {domain_vocab_size} tokens (mask_id={mask_token_id})")
    ...
    print("Building diffusion model (domain vocab)...")
    model = DiffusionWorldModel(
        config, st_dim, vocab,
    ...
    # Domain vocab: embeddings are trainable (not frozen T5)
    ...
    # Initialize embeddings from T5 via PCA
    if args.t5_init_embeddings:
        print("Initializing domain embeddings from T5 (PCA 768d -> 128d)...")
        t5_init = domain_tokenizer.build_t5_init_embeddings(target_dim=args.denoiser_dim)
        if args.unified_decoder:
            model.triple_decoder.token_emb.weight.data.copy_(t5_init)
        else:
            model.entity_decoder.token_emb.weight.data.copy_(t5_init)
            model.value_decoder.token_emb.weight.data.copy_(t5_init)
        print(f"  Initialized {t5_init.shape[0]} embeddings, dim={t5_init.shape[1]}")
