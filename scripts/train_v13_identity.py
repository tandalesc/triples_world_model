#!/usr/bin/env python3
"""Train v13: Identity advance — validate I/O through W-space.

No dynamics core. Encoder output feeds directly to decoder.
state_t == state_t+1, so the decoder reconstructs what the encoder sees.
This validates that the 256d W-space bottleneck carries enough information.

Curriculum:
  Phase 1: t in [0.7, 1.0] — 200 epochs
  Phase 2: t in [0.4, 1.0] — 200 epochs
  Phase 3: t in [0.0, 1.0] — 200 epochs (importance sampling)

Usage:
    uv run python scripts/train_v13_identity.py \
        --data-dir data/webnlg \
        --out-dir results/v13_identity_webnlg \
...
def _mse_loss(pred_emb, target_mask, target_ids, pad_mask, B, M, token_emb,
              pad_supervise_weight: float = 0.1):
    ...
    # Supervise pad positions: push predictions toward zero so NN decode → pad token
    is_pad = ~non_pad
    if is_pad.any():
        pad_pred = pred_emb[is_pad]
        pad_loss = (pad_pred ** 2).mean()
        loss = loss + pad_supervise_weight * pad_loss
...
def _ce_loss(logits, target_mask, target_ids, pad_mask, B, M,
    ...
    # Content tokens: CE ignoring pad positions
    loss = F.cross_entropy(logits_flat, targets_flat, ignore_index=0)

    # Pad token supervision: teach model to predict pad (token 0) at pad positions
    is_pad_tok = tgt_valid == 0
    if is_pad_tok.any():
        pad_logits = logits[is_pad_tok]
        pad_targets = torch.zeros(pad_logits.shape[0], dtype=torch.long, device=logits.device)
        pad_loss = F.cross_entropy(pad_logits, pad_targets)
        loss = loss + pad_supervise_weight * pad_loss
...
def run_eval_generation(
    ...
    for i in range(n):
        for m in range(M):
            ...
            if non_pad_e.any():
                tok_match = pred_e_tok[:len(tgt_e_ids)][non_pad_e] == tgt_e_ids[non_pad_e]
                ent_tok_correct += tok_match.sum().item()
                ent_tok_total += non_pad_e.sum().item()
                # Exact match = all non-pad target tokens correct
                if tok_match.all():
                    ent_exact += 1
            ...
            if non_pad_v.any():
                tok_match = pred_v_tok[:len(tgt_v_ids)][non_pad_v] == tgt_v_ids[non_pad_v]
...
def print_sample_predictions(
    ...
    """Print side-by-side target vs prediction with repr() and token IDs."""
    n = min(n_examples, len(ds))
    ...
    for i in range(n):
        for m in range(M):
            ...
            # Re-encode prediction to get token IDs for comparison
            pred_e_ids = domain_tokenizer.encode(pred_e, max_length=tgt_e_ids.shape[0])
            ...
            pred_v_ids = domain_tokenizer.encode(pred_v, max_length=tgt_v_ids.shape[0])

            e_match = "Y" if tgt_e == pred_e else "N"
            a_match = "Y" if tgt_a == pred_a else "N"
            v_match = "Y" if tgt_v == pred_v else "N"

            # Token ID comparison
            tgt_e_nonpad = [x for x in tgt_e_ids.tolist() if x != 0]
            pred_e_nonpad = [x for x in pred_e_ids if x != 0]
            tgt_v_nonpad = [x for x in tgt_v_ids.tolist() if x != 0]
            pred_v_nonpad = [x for x in pred_v_ids if x != 0]
            ...
            maxlen = max(len(tgt_v_nonpad), len(pred_v_nonpad))
            diffs = []
            for idx in range(maxlen):
                t = tgt_v_nonpad[idx] if idx < len(tgt_v_nonpad) else -1
                p = pred_v_nonpad[idx] if idx < len(pred_v_nonpad) else -1
                if t != p:
    ...
    for t_val in timesteps:
        ...
        if val_pred.shape[0] > 0 and val_mask.any():
            ...
            if non_pad.any():
                ...
                key = f"t_{t_val:.1f}_{'acc' if use_ce else 'cos'}"
                results[key] = 0.0
        else:
            key = f"t_{t_val:.1f}_{'acc' if use_ce else 'cos'}"
            results[key] = 0.0
...
def run_phase(
    ...
    print(f"\n{'='*60}")
    print(f"PHASE: {phase_name}  t in [{t_min}, {t_max}]")
    print(f"{'='*60}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        if epoch % log_every == 0 or epoch == 1:
            ...
            sweep_results = run_guidance_sweep(
                model, eval_ds, device, domain_tokenizer,
                n_examples=64, n_steps=denoise_steps, scales=guidance_scales,
            )
            ...
            if (diagnostic_every > 0 and epoch % diagnostic_every == 0):
                diag = run_loss_vs_timestep(model, eval_ds, device, n_examples=64)
                ...
                cond_diag = run_conditioning_reliance(model, eval_ds, device, n_examples=64)
            ...
            # Train reconstruction accuracy (identity-specific diagnostic)
            if epoch == 1 or epoch % diagnostic_every == 0:
                train_recon = run_eval_generation(
                    model, train_ds, device, domain_tokenizer,
                ...
                print(f"  RECON (train): val_tok={train_recon.get('gen_val_tok',0):.3f} "
                      ...
                      f"ent_ex={train_recon.get('gen_ent_exact',0):.3f}", flush=True)

                # Character-level prediction samples
...
def main():
    ap = argparse.ArgumentParser(description="Train v13: Identity advance I/O validation")
    ...
    ap.add_argument("--use-decode-proj", action="store_true",
                    help="Add decode_proj (Linear 256->256) with aux CE loss")
    ...
    print(f"NO DYNAMICS — identity advance (encoder output → decoder directly)")
    ...
    print(f"Building model (identity advance — no dynamics)...")
    model = DiffusionWorldModel(
        config, st_dim, vocab,
    ...
    # Resume from checkpoint if specified (e.g. for full-range continuation)
    if args.resume_checkpoint:
        ckpt_path = Path(args.resume_checkpoint)
        print(f"Loading resume checkpoint: {ckpt_path}")
        sd = torch.load(ckpt_path, map_location=device, weights_only=True)
        model.load_state_dict(sd, strict=False)
        print(f"  Loaded {len(sd)} state entries")

    # Identity advance: NO dynamics freezing, NO pretrained dynamics loading.
    # Dynamics is still in the model but we train encoder + decoder + dynamics together.
    # The encoder projection (384→256) is trainable.
    # The MiniLM backbone is frozen (it's in sentence-transformers, never part of our model).
    ...
    p1_best = out_dir / "phase1" / "model_best.pt"
    if p1_best.exists():
    ...
    # Phase 3: t in [0.0, 1.0] with importance sampling
    gvt3, ep3 = run_phase(
        phase_name="phase3",
        t_min=0.0, t_max=1.0,
    ...
    best_gvt = max(gvt1, gvt2, gvt3)
    if best_gvt == gvt3 and (out_dir / "phase3" / "model_best.pt").exists():
        best_phase = "phase3"
    elif best_gvt == gvt2 and (out_dir / "phase2" / "model_best.pt").exists():
        best_phase = "phase2"
    else:
        best_phase = "phase1"
    ...
    print(f"\n{'='*60}")
    print(f"SUMMARY — Identity Advance I/O Validation")
    print(f"  Phase 1 (t in [0.7,1.0]): best gvt={gvt1:.4f}")
    print(f"  Phase 2 (t in [0.4,1.0]): best gvt={gvt2:.4f}")
    print(f"  Phase 3 (t in [0.0,1.0]): best gvt={gvt3:.4f}")
    print(f"{'='*60}")
