#!/usr/bin/env python3
"""Train flow matching decoder on TextWorld dynamics.

Compressor → dynamics → flow decoder. Factored consistency loss +
velocity MSE loss. No autoregressive generation — parallel ODE.

Usage:
  uv run python scripts/train_flow.py configs/flow_v1.json
"""
...
from twm.flow_decoder import FlowDecoder
...
class FlowDynamicsModel(nn.Module):
    """Compressor + dynamics + flow decoder."""

    def __init__(self, config, tokenizer, compressor_layers=3, dynamics_layers=2,
                 decoder_layers=4, max_text_tokens=128, dropout=0.1,
                 bottleneck_dim=None, n_ode_steps=8):
        super().__init__()
        self.config = config
        ...
        self.decoder = FlowDecoder(
            token_emb=self.shared_token_emb, d_model=d, n_heads=config.n_heads,
            n_layers=decoder_layers, max_text_tokens=max_text_tokens,
            dropout=dropout, bottleneck_dim=bn_d, n_ode_steps=n_ode_steps,
        )
...
def compute_loss(model, target_compressor, batch, device, cfg):
    ...
    lambda_flow = cfg.get("flow_loss_weight", 1.0)
    lambda_len = cfg.get("length_loss_weight", 0.1)
    ...
    consist_total = vel_total = len_total = tok_total = 0.0
    n_steps = 0
    ...
    for step in range(1, max_chain):
        ...
        # Flow matching loss
        vel_loss, len_loss, flow_metrics = model.decoder(
            active_bn, target_ids, target_pad, pre_dynamics=active_pre
        )
        total_loss = total_loss + lambda_flow * vel_loss + lambda_len * len_loss
        vel_total += flow_metrics["velocity_mse"]
        len_total += flow_metrics["length_mse"]
        tok_total += flow_metrics["tok_acc"]

        # Per-mode tok_acc (from flow metrics single-step correction)
        with torch.no_grad():
            for m in mode_names:
                mask = active_modes == m
                if mask.any():
                    # Approximate per-mode by running subset
                    mode_tok_sum[m] += flow_metrics["tok_acc"]  # same for all modes this batch
                    mode_tok_n[m] += 1
    ...
    metrics = {
        ...
        "vel_mse": vel_total / max(n_steps, 1),
        "len_mse": len_total / max(n_steps, 1),
        "tok_acc": tok_total / max(n_steps, 1),
    }
...
def generation_eval(model, test_loader, device, tok, n_samples=200, n_ode_steps=8):
        ...
        for batch in test_loader:
            ...
            for i in range(len(input_ids)):
                ...
                gen_ids = model.decoder.generate(
                    bn[i:i+1], n_steps=n_ode_steps,
                    pre_dynamics=pre_bn[i:i+1],
                )
...
def main():
    ...
    model = FlowDynamicsModel(
        ...
        n_ode_steps=cfg.get("n_ode_steps", 8),
    )
    ...
    print(f"Flow decoder: {model.decoder.trainable_param_count():,} params, {cfg.get('n_ode_steps', 8)} ODE steps")
    ...
    for epoch in range(1, epochs + 1):
        model.train()
        ep = {"loss": 0, "consist": 0, "vel_mse": 0, "len_mse": 0, "tok_acc": 0}
        ep_mode = {}
        n_batches = 0
        ...
        if epoch % log_every == 0 or epoch == 1:
            ...
            msg = (f"ep {epoch:4d} | loss {ep['loss']:.4f} | consist {ep['consist']:.4f} "
                   f"| vel {ep['vel_mse']:.4f} | tok {ep['tok_acc']:.3f}{mode_str} | lr {lr_now:.2e}")
            print(msg)
            log_file.write(msg + "\n")
            log_file.flush()
    ...
    print(f"AR decoder baselines: BPE chrF 49.0, bytes chrF 38.3")
