...
Phase A (1-20): GT entity prefix, entity head trains end-to-end.
Phase B (21-40): Predicted entity prefix, tests entity head accuracy.
...
def extract_entity_targets(text, entity_head):
    ...
    # Place — check rooms and locations
    place_idx = -1
...
def make_prefix_tokens(action_str, object_str, place_str, tokenizer, max_prefix=20):
    """Convert entity strings to BPE token IDs for AR decoder prefix."""
    prefix_text = f"{action_str} | {object_str} | {place_str}"
    ids = tokenizer.encode(prefix_text, max_length=max_prefix)
    # Remove padding
...
def compute_loss(model, target_compressor, batch, device, cfg, epoch):
    ...
    phase_b_epoch = cfg.get("phase_b_epoch", 21)
    use_predicted = epoch >= phase_b_epoch
    ...
    for step in range(1, max_chain):
        ...
        target_ids_batch = chain_ids[active, step]
        target_pad_batch = chain_pad[active, step]
        ...
        # Consistency loss
        with torch.no_grad():
            target_bn = target_compressor(target_ids_batch, target_pad_batch, model.config.max_triples)
        ...
        # Extract entity targets from text
        ...
        for b in range(B_active):
            non_pad = ~target_pad_batch[b]
            text = tok.decode(target_ids_batch[b][non_pad].tolist())
            ...
            # Build prefix tokens
            a_str = eh.actions[a_idx] if a_idx >= 0 else "none"
            o_str = eh.objects[o_idx] if o_idx >= 0 else "none"
            p_str = eh.places[p_idx] if p_idx >= 0 else "none"

            if use_predicted:
                preds = eh.predict(active_bn[b:b+1])
                a_str = preds["action"][0]
                o_str = preds["object"][0]
                p_str = preds["place"][0]

            ids = make_prefix_tokens(a_str, o_str, p_str, tok)
        ...
        # Pad prefix tokens to same length
        max_prefix_len = max(len(ids) for ids in prefix_ids_list)
        prefix_padded = torch.full((B_active, max_prefix_len), tok.pad_token_id, dtype=torch.long, device=device)
        for b, ids in enumerate(prefix_ids_list):
            prefix_padded[b, :len(ids)] = torch.tensor(ids, device=device)

        # AR decode with entity prefix
        logits = model.decoder(
            active_bn, target_ids_batch, target_pad_batch,
            pre_dynamics=active_pre, entity_prefix=prefix_padded,
        )

        # EOS-aware CE loss
        eos_mask = ~target_pad_batch.clone()
        for b in range(B_active):
            pad_pos = target_pad_batch[b].nonzero(as_tuple=True)[0]
            if len(pad_pos) > 0:
                eos_mask[b, pad_pos[0]] = True
        if eos_mask.any():
            ce_loss = F.cross_entropy(logits[eos_mask], target_ids_batch[eos_mask])
            total_loss = total_loss + ce_loss

        # Metrics
        with torch.no_grad():
            non_pad = ~target_pad_batch
            if non_pad.any():
                preds = logits[non_pad].argmax(-1)
                tok_acc = (preds == target_ids_batch[non_pad]).float().mean().item()
                total_tok_acc += tok_acc
                for m in mode_names:
                    mask = active_modes == m
                    if mask.any():
                        m_np = ~target_pad_batch[mask]
                        if m_np.any():
                            mp = logits[mask][m_np].argmax(-1)
                            mt = target_ids_batch[mask][m_np]
    ...
    with torch.no_grad():
        for batch in test_loader:
            ...
            for i in range(len(input_ids)):
                ...
                # Predict entities
                prefix_strs = model.entity_head.predict_prefix_strings(bn[i:i+1])
                prefix_ids = make_prefix_tokens(
                    *prefix_strs[0].split(" | "), tok
                )
...
def main():
    ...
    print(f"Entity head: act={model.entity_head.n_actions}, obj={model.entity_head.n_objects}, place={model.entity_head.n_places}")
    print(f"Phase B at epoch {cfg.get('phase_b_epoch', 21)}")
    ...
    for epoch in range(1, epochs + 1):
        ...
        phase = "A (GT prefix)" if epoch < cfg.get("phase_b_epoch", 21) else "B (predicted prefix)"
        ...
        if epoch % eval_every == 0:
            ...
            for ex in examples[:3]:
                ...
                print(f"  [{mode_name}] prefix: {ex['prefix']}")
                print(f"    ref: {ex['ref']}")
                print(f"    gen: {ex['gen']}")
                log_file.write(f"  [{mode_name}] prefix: {ex['prefix']}\n")
    ...
    print(f"Phase 2 baseline (no entity head): chrF 49.0")
